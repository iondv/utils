...
module.exports = function (dst, options) {
  ...
  function exportData(dst, cm, fileDir, skipFiles, offset) {
    ...
    return options.dataRepo.getList(
      ...
      (list) => {
        ...
        if (list.length === step) {
          return w.then(() => exportData(dst, cm, fileDir, skipFiles, ofst + step));
        } else {
          return w;
        }
      }
  ...
  function exportClass(dst, cm, paths, metaRepo, noVersion) {
    log(format(t('Exporting class %s'), cm.getCanonicalName()));
    ...
    .then(() => {
      ...
      metas.forEach((cm) => {
        ...
        if (!options.skipData && !cm.getAncestor()) {
          p = p.then(() => {
            log(format(t('Exporting data for class %s'), cm.getCanonicalName()));
      ...
      if (!options.skipData) {
        p = p
          ...
          .then(snapshot =>
            new Promise((resolve, reject) => {
              log(t('Exporting named sequences'));
