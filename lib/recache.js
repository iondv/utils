...
module.exports = function (classes, options, metaRepo, dataRepo, log) {
    ...
    return dataRepo
      ...
      .then((list) => {
        ...
        if (total % 100 == 0 || list.length < step) {
          promise = promise.then(() => log && log(format(t('%s objects processed'), total)));
        }
        if (list.length === step) {
          promise = promise
  ...
  try {
    log && log(t('Cache recalculation started'));
    ...
    classList.forEach((cn) => {
      ...
      cachedClasses.forEach((cn) => {
        classPromise = classPromise.then(() => {
          log && log(format(t('Recalculating objects of class %s'), cn));
          return recacheChunk(dataRepo, cn, {skipCacheDependencies: skipDep}, log, {step, sort});
        });
      });
    });
    return classPromise.then(() => log && log(t('Recalculation complete!')));
  } catch (e) {
